...
import lossByTime from './lossByTime';
import attemptLoss from './attemptLoss';
...
const renderRandomScreen = () => {
  const screens = [winResult, lossByTime, attemptLoss];
...
const answersBtn = [...screen.querySelectorAll(`.artist`)];

const welcomeBackBtn = screen.querySelector(`.game__back`);
welcomeBackBtn.addEventListener(`click`, () => renderScreen(welcome));

answersBtn.forEach((btn) => btn.addEventListener(`click`, renderRandomScreen));
